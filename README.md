...
유저 스토리는 **사용자 관점에서 소프트웨어가 제공해야 할 기능과 가치를 짧게 표현한 요구사항**이다. 주로 애자일 개발에서 사용한다.

### 기본 형식

> **[사용자 역할]로서, [목표]를 하고 싶다. 그래야 [가치·이유]를 얻을 수 있다.**

### 예시

> **온라인 쇼핑몰 고객으로서, 장바구니의 총금액을 확인하고 싶다. 그래야 예산을 초과하지 않을 수 있다.**
...
- 상품의 가격과 수량이 총금액에 반영된다.
- 할인과 배송비가 별도로 표시된다.
- 수량을 변경하면 총금액이 즉시 갱신된다.

### 좋은 유저 스토리의 INVEST 원칙

- **Independent:** 다른 스토리와 독립적이어야 한다.
- **Negotiable:** 구현 방법을 협의할 수 있어야 한다.
- **Valuable:** 사용자에게 가치가 있어야 한다.
- **Estimable:** 개발 규모를 추정할 수 있어야 한다.
- **Small:** 짧은 기간에 완료할 만큼 작아야 한다.
- **Testable:** 완료 여부를 시험할 수 있어야 한다.

### 핵심 요약

유저 스토리는 **누가, 무엇을 원하고, 왜 필요한지**를 사용자 관점에서 표현하는 요구사항 작성 방식이다.
